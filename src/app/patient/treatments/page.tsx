...
const TreatmentsPage: React.FC = () => {
  ...
  const [form, setForm] = useState<any>({ patientId: '', doctorId: '', treatmentName: '', treatmentDate: '', notes: '', outcome: '' });
  ...
  const fetchItems = async () => { setLoading(true); try { const r = await fetch('/api/treatments'); const d = await r.json().catch(()=>[]); setItems(Array.isArray(d)?d:[]);} catch{ setItems([]);} finally { setLoading(false);} };
  useEffect(()=>{ fetchPatients(); fetchDoctors(); fetchItems(); }, []);

  const filtered = useMemo(()=>{ const q = filterQ.trim().toLowerCase(); if(!q) return items; return items.filter((it:any)=> [it.treatmentName, it.notes, it.outcome, it.patient?.firstName, it.patient?.lastName, it.doctor?.firstName, it.doctor?.lastName].some((v:any)=> String(v||'').toLowerCase().includes(q))); }, [items, filterQ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (payload.doctorId) payload.doctorId = Number(payload.doctorId); // optional; API will infer if missing using username
      if (payload.treatmentDate) payload.treatmentDate = new Date(payload.treatmentDate).toISOString();
      const res = await apiFetch('/api/treatments', { method: editingId? 'PUT':'POST', body: JSON.stringify(editingId? { id: editingId, ...payload }: payload) });
      const result = await res.json().catch(()=>({}));
      if (!res.ok) setMessage({ type:'error', text: safeMsg(result)||'Server error' });
      else { setMessage({ type:'success', text: editingId? 'Updated':'Created' }); setForm({ patientId:'', doctorId:'', treatmentName:'', treatmentDate:'', notes:'', outcome:'' }); setEditingId(null); await fetchItems(); }
    } catch { setMessage({ type:'error', text:'Request failed' }); }
    setLoading(false);
  };
  const handleEdit = (it:any)=>{ setForm({ patientId: it.patientId? String(it.patientId):'', doctorId: it.doctorId? String(it.doctorId):'', treatmentName: it.treatmentName||'', treatmentDate: it.treatmentDate? new Date(it.treatmentDate).toISOString().slice(0,16):'', notes: it.notes||'', outcome: it.outcome||'' }); setEditingId(it.id); };
  const handleDelete = async (id:number)=>{ if(!confirm('Delete treatment?')) return; setLoading(true); try { const res = await apiFetch('/api/treatments', { method:'DELETE', body: JSON.stringify({ id }) }); const r = await res.json().catch(()=>({})); if(!res.ok) setMessage({ type:'error', text: safeMsg(r)||'Delete failed' }); else setMessage({ type:'success', text:'Deleted' }); await fetchItems(); } catch { setMessage({ type:'error', text:'Delete failed' }); } setLoading(false); };
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8">
        ...
        <div className="mb-4 flex flex-col md:flex-row gap-2 items-center">
          <input placeholder="Search patient/doctor/treatment" value={filterQ} onChange={e=>{ setFilterQ(e.target.value); setPage(1); }} className="border p-2 rounded w-full md:flex-1" />
        ...
        <form onSubmit={handleSubmit} className="mb-6 bg-white p-4 rounded shadow grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <input type="datetime-local" className="border p-2 rounded" value={form.treatmentDate} onChange={e=>setForm({ ...form, treatmentDate: e.target.value })} required />
          <input className="border p-2 rounded md:col-span-2" placeholder="Treatment Name" value={form.treatmentName} onChange={e=>setForm({ ...form, treatmentName: e.target.value })} required />
          ...
          <input className="border p-2 rounded md:col-span-3" placeholder="Outcome" value={form.outcome} onChange={e=>setForm({ ...form, outcome: e.target.value })} />
          <div className="md:col-span-3 flex gap-2 justify-end">
            <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded">{editingId? 'Update':'Create'}</button>
            {editingId && <button type="button" className="bg-gray-400 text-white px-4 py-2 rounded" onClick={()=>{ setEditingId(null); setForm({ patientId:'', doctorId:'', treatmentName:'', treatmentDate:'', notes:'', outcome:'' }); }}>Cancel</button>}
          </div>
        </form>
            ...
            <tbody>
              {pageItems.map((it:any)=> (
                <tr key={it.id} className="border-t">
                  ...
                  <td className="p-2 hidden md:table-cell">{it.treatmentName}</td>
                  <td className="p-2 hidden md:table-cell">{it.notes}</td>
                  <td className="p-2 hidden md:table-cell">{it.outcome}</td>
