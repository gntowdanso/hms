...
const DiagnosesPage: React.FC = () => {
  ...
  const [form, setForm] = useState<any>({ patientId: '', doctorId: '', date: '', condition: '', notes: '' });
  ...
  const fetchItems = async () => { setLoading(true); try { const r = await fetch('/api/diagnoses'); const d = await r.json().catch(()=>[]); setItems(Array.isArray(d)?d:[]);} catch{ setItems([]);} finally { setLoading(false);} };
  useEffect(()=>{ fetchPatients(); fetchDoctors(); fetchItems(); }, []);

  const filtered = useMemo(()=>{ const q = filterQ.trim().toLowerCase(); if(!q) return items; return items.filter((it:any)=> [it.condition, it.notes, it.patient?.firstName, it.patient?.lastName, it.doctor?.firstName, it.doctor?.lastName].some((v:any)=> String(v||'').toLowerCase().includes(q))); }, [items, filterQ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiFetch('/api/diagnoses', { method: editingId? 'PUT':'POST', body: JSON.stringify(editingId? { id: editingId, ...payload }: payload) });
      const result = await res.json().catch(()=>({}));
      if (!res.ok) setMessage({ type:'error', text: safeMsg(result)||'Server error' });
      else { setMessage({ type:'success', text: editingId? 'Updated':'Created' }); setForm({ patientId:'', doctorId:'', date:'', condition:'', notes:'' }); setEditingId(null); await fetchItems(); }
    } catch { setMessage({ type:'error', text:'Request failed' }); }
    setLoading(false);
  };
  const handleEdit = (it:any)=>{ setForm({ patientId: it.patientId? String(it.patientId):'', doctorId: it.doctorId? String(it.doctorId):'', date: it.date? new Date(it.date).toISOString().slice(0,16):'', condition: it.condition||'', notes: it.notes||'' }); setEditingId(it.id); };
  const handleDelete = async (id:number)=>{ if(!confirm('Delete diagnosis?')) return; setLoading(true); try { const res = await apiFetch('/api/diagnoses', { method:'DELETE', body: JSON.stringify({ id }) }); const r = await res.json().catch(()=>({})); if(!res.ok) setMessage({ type:'error', text: safeMsg(r)||'Delete failed' }); else setMessage({ type:'success', text:'Deleted' }); await fetchItems(); } catch { setMessage({ type:'error', text:'Delete failed' }); } setLoading(false); };
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8">
        ...
        <div className="mb-4 flex flex-col md:flex-row gap-2 items-center">
          <input placeholder="Search patient/doctor/condition" value={filterQ} onChange={e=>{ setFilterQ(e.target.value); setPage(1); }} className="border p-2 rounded w-full md:flex-1" />
        ...
        <form onSubmit={handleSubmit} className="mb-6 bg-white p-4 rounded shadow grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <input className="border p-2 rounded md:col-span-2" placeholder="Condition" value={form.condition} onChange={e=>setForm({ ...form, condition: e.target.value })} required />
          ...
          <div className="md:col-span-3 flex gap-2 justify-end">
            ...
            {editingId && <button type="button" className="bg-gray-400 text-white px-4 py-2 rounded" onClick={()=>{ setEditingId(null); setForm({ patientId:'', doctorId:'', date:'', condition:'', notes:'' }); }}>Cancel</button>}
          </div>
        </form>
            ...
            <tbody>
              {pageItems.map((it:any)=> (
                <tr key={it.id} className="border-t">
                  ...
                  <td className="p-2 hidden md:table-cell">{it.condition}</td>
